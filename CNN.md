...
이미지 처리 및 CNN에서 자주 사용되는 **경계 검출, 샤프닝, 블러** 필터의 구조와 특징을 한눈에 볼 수 있도록 표로 정리했습니다. README.md 등에서 바로 활용할 수 있는 형태입니다.

---

# 수식에서 자주 나오는 용어(기호) 설명

필터 공식에서 사용되는 `frac`, `dot`과 같은 용어(기호)는 수식 표현을 위해 자주 등장합니다. 각각의 의미와 실제 표현 방식을 아래에 정리합니다.

---

#### `frac`

- **의미:** 분수(fraction)를 나타내는 기호입니다.
- **풀어서 설명:**  
  - `\frac{a}{b}`는 "a를 b로 나눈 값", 즉 a/b(분수)를 의미합니다.
  - 예시:  
    - \( \frac{1}{16} \) → "16분의 1"
...
- **의미:** 곱셈(특히 스칼라 곱 또는 내적, 곱하기 연산)을 나타내는 기호입니다.
- **풀어서 설명:**  
  - `\cdot`은 "곱하기"를 의미하며, 두 값 사이의 곱셈을 명확히 표시할 때 사용합니다.
  - 예시:  
...
- **\(\sum\) (시그마):**  
  - "합(sum)"을 의미하며, 여러 값을 모두 더할 때 사용합니다.
  - 예시: \( \sum_{i=1}^{9} z_i \) → "z_1부터 z_9까지 모두 더한 값"

- **\(z_1, z_2, ..., z_9\):**  
  - 3×3 영역의 각 픽셀 값을 의미합니다.  
  - \(z_5\)는 중앙 픽셀, 나머지는 주변 8개 픽셀입니다.
